...
rms_disp_sdof_base = np.sqrt(np.mean(floor_disp**2))
...
for config_num, tmd_config in enumerate(tmd_configurations):
    ...
    # TMD parameters - MOVED THESE CALCULATIONS HERE
    md = mu * ms
...
# ==============================================================================
## FINAL: Plotting All Comparison Results & Reduction Rate Analysis
# ==============================================================================
print("\n" + "="*50)
...
# --- Calculate and print Mean, RMS, Peak values for Floor Displacement with Reduction Rate ---
print("\n" + "="*80)
print("--- Performance Metrics for Main Structure Displacement (Relative to Ground) with Reduction Rate ---")
print("="*80)
print("{:<45} {:<15} {:<15} {:<15} {:<15} {:<15} {:<15}".format(
    "Structure Configuration", "Mean (m)", "RMS (m)", "Peak (m)",
    "Reduction (Mean) (%)", "Reduction (RMS) (%)", "Reduction (Peak) (%)"
))
...
for label, df in all_results_dfs.items():
    ...
    # Calculate reduction rate only for TMD configurations
    if label == 'Single Floor (No TMD)':
        reduction_mean = "N/A"
        reduction_rms = "N/A"
        reduction_peak = "N/A"
    else:
        # Reduction rate = ((Value_NoTMD - Value_TMD) / Value_NoTMD) * 100%
        # A positive value means reduction.
        reduction_mean = ((mean_disp_sdof_base - mean_disp) / mean_disp_sdof_base) * 100
        reduction_rms = ((rms_disp_sdof_base - rms_disp) / rms_disp_sdof_base) * 100
        reduction_peak = ((peak_disp_sdof_base - peak_disp) / peak_disp_sdof_base) * 100
        reduction_mean = f"{reduction_mean:.2f}"
        reduction_rms = f"{reduction_rms:.2f}"
        reduction_peak = f"{reduction_peak:.2f}"


    print(f"{label:<45} {mean_disp:<15.6f} {rms_disp:<15.6f} {peak_disp:<15.6f} "
          f"{reduction_mean:<20} {reduction_rms:<20} {reduction_peak:<15}")
